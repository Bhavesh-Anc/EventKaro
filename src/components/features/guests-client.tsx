...
import { Users, List, Truck, Plus, Filter, Search, Upload, Send, Download } from 'lucide-react';
...
export function GuestsClient({
  ...
  const handleExportPDF = () => {
    const formatDate = (d: string) => new Date(d).toLocaleDateString('en-IN', { dateStyle: 'long' });
    const statusEmoji = (s: string) => s === 'accepted' ? '✓' : s === 'declined' ? '✗' : s === 'maybe' ? '?' : '○';

    const html = `
      <!DOCTYPE html>
      <html>
        <head>
          <title>Guest List - ${eventName || 'Wedding'}</title>
          <style>
            body { font-family: system-ui, sans-serif; max-width: 900px; margin: 0 auto; padding: 20px; }
            h1 { color: #9f1239; border-bottom: 2px solid #9f1239; padding-bottom: 10px; }
            ...
            th { background: #9f1239; color: white; }
        ...
        <body>
          <h1>Guest List</h1>
          <div class="meta">
            <strong>${eventName || 'Wedding Celebration'}</strong>
            ${eventDate ? ` • ${formatDate(eventDate)}` : ''}
            <br/>Generated on ${new Date().toLocaleDateString('en-IN', { dateStyle: 'full' })}
          </div>
          <div class="stats">
            <div class="stat">
              <div class="stat-value">${rsvpStats?.totalGuests || individuals.length}</div>
              <div class="stat-label">Total Guests</div>
            </div>
            <div class="stat">
              <div class="stat-value">${rsvpStats?.confirmedGuests || 0}</div>
              <div class="stat-label">Confirmed</div>
            </div>
            <div class="stat">
              <div class="stat-value">${rsvpStats?.pendingGuests || 0}</div>
              <div class="stat-label">Pending</div>
            </div>
            <div class="stat">
              <div class="stat-value">${rsvpStats?.declinedGuests || 0}</div>
          ...
          <table>
            ...
            <tbody>
              ${individuals.map(g => `
                <tr>
                  <td>${g.name}${g.is_vip ? ' ⭐' : ''}${g.is_elderly ? ' 👴' : ''}${g.is_child ? ' 👶' : ''}</td>
                  <td>${g.family_name}</td>
                  <td>${g.family_side === 'bride' ? "Bride's" : "Groom's"}</td>
                  <td class="status-${g.rsvp_status}">${statusEmoji(g.rsvp_status)} ${g.rsvp_status}</td>
                  <td>${g.phone || '-'}</td>
                  <td>${g.is_outstation ? 'Outstation' : ''}${g.hotel_assigned ? ' • Hotel' : ''}${g.pickup_assigned ? ' • Pickup' : ''}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
          <div class="footer">
            Generated by EventKaro • Total: ${individuals.length} guests
          </div>
        </body>
      </html>
    `;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2 rounded-lg border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 flex items-center gap-2 transition-all"
          >
