...
import { ChevronDown, ChevronUp, TrendingUp, TrendingDown, DollarSign, AlertTriangle, Users, AlertCircle, Clock, Lightbulb, FileText } from 'lucide-react';
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border-2 border-gray-200 bg-white p-5">
          ...
          <div className="space-y-3">
            {summary.health === 'over-budget' && (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-red-50 border border-red-200">
                <span className="text-xl">🔴</span>
                <div>
                  <div className="text-sm font-semibold text-red-900">Over Budget</div>
                  <div className="text-xs text-red-700">
                    Committed exceeds total budget by {formatFullINR(summary.overrun)}
                  </div>
                </div>
              </div>
            )}
            {summary.pending > summary.totalBudget * 0.3 && (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
                <span className="text-xl">🟠</span>
                <div>
                  <div className="text-sm font-semibold text-amber-900">High Pending Exposure</div>
                  <div className="text-xs text-amber-700">
                    {formatFullINR(summary.pending)} pending payments ({Math.round((summary.pending / summary.totalBudget) * 100)}% of budget)
                  </div>
                </div>
              </div>
            )}
            {summary.paid < summary.committed * 0.3 && summary.committed > 0 && (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
                <span className="text-xl">🟠</span>
                <div>
                  <div className="text-sm font-semibold text-amber-900">Low Advance Payments</div>
                  <div className="text-xs text-amber-700">
                    Only {Math.round((summary.paid / summary.committed) * 100)}% paid of committed amount
                  </div>
                </div>
              </div>
            )}
            {categories.filter(c => c.isOverBudget).length > 0 && (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
                <span className="text-xl">🟠</span>
                <div>
                  <div className="text-sm font-semibold text-amber-900">{categories.filter(c => c.isOverBudget).length} Categories Over Budget</div>
                  <div className="text-xs text-amber-700">
                    {categories.filter(c => c.isOverBudget).map(c => c.category.replace(/_/g, ' ')).join(', ')}
                  </div>
                </div>
              </div>
            )}
            {summary.health === 'on-track' && categories.filter(c => c.isOverBudget).length === 0 && (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-green-50 border border-green-200">
                <span className="text-xl">✅</span>
                <div>
                  <div className="text-sm font-semibold text-green-900">All Clear</div>
                  <div className="text-xs text-green-700">No budget risks detected</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="rounded-xl border-2 border-gray-200 bg-white p-5">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 p-3 rounded-lg bg-blue-50 border border-blue-200">
              ...
              <div>
                <div className="text-sm font-semibold text-blue-900">Guest Impact</div>
                <div className="text-xs text-blue-700">
                  Every +10 guests ≈ +₹28,400 (catering + transport)
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-lg bg-purple-50 border border-purple-200">
              ...
              <div>
                <div className="text-sm font-semibold text-purple-900">Vendor Scope</div>
                <div className="text-xs text-purple-700">
                  1 additional photographer ≈ +₹65,000
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-lg bg-rose-50 border border-rose-200">
              ...
              <div>
                <div className="text-sm font-semibold text-rose-900">Timeline Buffer</div>
                <div className="text-xs text-rose-700">
                  Late changes cost 15-20% more on average
                </div>
              </div>
            </div>
          </div>
          <Link
            href="/guests"
            className="mt-4 block text-center text-sm text-gray-600 hover:text-gray-900 font-semibold"
          >
            Manage Guests →
          </Link>
        </div>
            ...
            >
              ...
              {isExpanded && (
                <div className="px-6 pb-6 border-t border-gray-200 pt-4 space-y-4">
                  {/* Vendor Entries with Payment States */}
                  {categoryEntries.length > 0 ? (
                    <div className="space-y-3">
                      ...
                      {categoryEntries.map((entry: any, idx: number) => {
                        ...
                        const committed = entry.committed_amount_inr || 0;
                        const paid = entry.paid_amount_inr || 0;
                        const pending = entry.pending_amount_inr || 0;

                        let paymentState: 'not_due' | 'advance_due' | 'partial' | 'full' | 'overpaid' = 'not_due';
                        let stateLabel = 'Not Due';
                        ...
                        } else if (paid === committed && committed > 0) {
                          paymentState = 'full';
                          stateLabel = 'Fully Paid';
                          stateColor = 'bg-green-100 text-green-700';
                        } else if (paid > 0 && paid < committed) {
                          paymentState = 'partial';
                          stateLabel = 'Partially Paid';
                          stateColor = 'bg-blue-100 text-blue-700';
                        } else if (committed > 0 && paid === 0) {
                          paymentState = 'advance_due';
                          stateLabel = 'Advance Due';
                          stateColor = 'bg-amber-100 text-amber-700';
                        }

                        const paidPercent = committed > 0 ? Math.round((paid / committed) * 100) : 0;
                          ...
                          >
                            <div className="p-4">
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex-1">
                                  ...
                                  <div className="text-sm text-gray-600">
                                    {entry.wedding_events?.event_name || 'Event'} • {entry.wedding_events?.custom_event_name || ''}
                                  </div>
                                </div>
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${stateColor}`}>
                                  {stateLabel}
                                </span>
                              </div>
                              ...
                              <div className="h-2 w-full rounded-full bg-gray-200 mb-3">
                                <div
                                  className={`h-full rounded-full transition-all ${
                                    paymentState === 'full' ? 'bg-green-500' :
                                    paymentState === 'overpaid' ? 'bg-purple-500' :
                                    paymentState === 'partial' ? 'bg-blue-500' :
                                    'bg-amber-500'
                                  }`}
                                  style={{ width: `${Math.min(paidPercent, 100)}%` }}
                                />
                              ...
                              <div className="grid grid-cols-3 gap-4 text-sm">
                                <div>
                                  <div className="text-gray-500 text-xs">Committed</div>
                                  <div className="font-bold text-gray-900">
                                    {formatFullINR(committed)}
                                  </div>
                                </div>
                                <div>
                                  <div className="text-gray-500 text-xs">Paid ({paidPercent}%)</div>
                            ...
                            {/* Payment Due Warning */}
                            {paymentState === 'advance_due' && (
                              <div className="px-4 py-2 bg-amber-50 border-t border-amber-200 flex items-center gap-2 text-amber-800 text-xs">
                                <Clock className="h-3 w-3" />
                                <span className="font-semibold">Advance payment pending</span>
                              </div>
                            )}
                          </div>
                        );
